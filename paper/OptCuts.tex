...
\centering
  ...
  continuous changes in the embedding vertices and discrete topological changes in the UV mesh by propagating simple and local topology splitting and merging operations. In this example a distortion bound of $4.1$ is enforced, measured by the symmetric Dirichlet distortion energy~\cite{Smith2015Bijective}; we report total inner iterations at each rendered frame. }
  \label{fig:teaser}
\end{teaserfigure}

\begin{abstract}
% context and need
Low-distortion mapping of three-dimensional surfaces to the plane is a critical problem in geometry processing. The intrinsic distortion introduced by these UV mappings is highly dependent on the choice of surface cuts that form seamlines 
...
%
We propose {\em OptCuts}, an algorithm that jointly optimizes the parameterization and cutting of a three-dimensional mesh. 
