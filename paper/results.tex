...
In the following sections we evaluate OptCuts and a range of other UV parameterization methods and commercial tools on meshes from a benchmark mesh dataset containing a diverse range of \old{71} \revised{74} surfaces commonly employed in geometry processing research. Meshes in this benchmark are organic shapes with intricate geometric details and varying resolutions ranging from 80 to 10K vertices (with 3.7K average vertex count); we also employ a set of refined Lucy models with resolutions increasing from 2.5K to 48K vertices.
...
All our evaluations in the following sections employ our implementation of OptCuts using the libigl~\cite{libigl} library for rapid prototyping and testing. Implementation details are reported above in Section~\ref{sec:imp}. We \old{will} release our implementation publicly for future development and application. All experiments presented in this paper were executed on a Macbook Pro with \old{3.1 GHz quad-core Intel Core i7 CPU and 16 GB 2133 MHz LPDDR3 memory}\revised{2.5 GHz quad-core Intel Core i7 CPU and 16 GB 1600 MHz DDR3 memory}.
...
\multirow{2}{*}{4.2}   & OFF                    & \shortstack{\old{3.819}\\\corr{3.936}\\\revised{3.816}}   & \shortstack{\old{0.080}\\\cor{0.289}}  & \shortstack{\old{14.545}\\\revised{14.579}}  & \shortstack{\old{87.0}\\\corr{92.5}\\\revised{38.4}}   & \shortstack{\old{0.3}\\\revised{0.2}} & \shortstack{\old{417.8}\\\revised{235.6}} \\
                       & ON                & \shortstack{\old{4.087}\\\corr{4.006}\\\revised{3.915}}   & 0.289  & \shortstack{\old{17.063}\\\revised{17.243}}  & \shortstack{\old{190.3}\\\corr{187.0}\\\revised{69.5}}   & \shortstack{\old{3.8}\\\corr{0.6}\\\revised{0.5}} & \shortstack{\old{983.3}\\\revised{438.5}}  \\ \hline
\multirow{2}{*}{4.1}   & OFF                    & \shortstack{\old{4.709}\\\corr{4.919}\\\revised{4.783}}   & 0.752  & \shortstack{\old{17.980}\\\revised{17.786}}  & \shortstack{\old{137.5}\\\corr{144.6}\\\revised{42.5}}  & \shortstack{\old{0.9}\\\corr{4.0}\\\revised{0.4}} & \shortstack{\old{886.9}\\\revised{206.2}} \\
                       & ON                & \shortstack{\old{5.324}\\\corr{5.346}\\\revised{5.147}}   & 0.860  & \shortstack{\old{21.595}\\\revised{22.009}}  & \shortstack{\old{271.1}\\\corr{274.6}\\\revised{88.1}}   & \shortstack{\old{6.9}\\\revised{0.8}} & \shortstack{\old{1767.8}\\\revised{458.4}}  \\ \hline
\multirow{2}{*}{4.05}  & OFF                    & \shortstack{\old{6.142}\\\corr{6.416}\\\revised{6.190}}   & 0.277  & 21.566  & \shortstack{\old{213.2}\\\corr{223.3}\\\revised{57.0}}  & \shortstack{\old{3.9}\\\revised{0.4}} & \shortstack{\old{1398.1}\\\revised{245.9}}   \\
                       & ON                & \shortstack{\old{7.195}\\\corr{7.256}\\\revised{6.980}}   & \shortstack{\old{0.932}\\\revised{0.781}}  & \shortstack{\old{29.596}\\\revised{30.156}}  & \shortstack{\old{412.9}\\\corr{479.5}\\\revised{119.7}}   & \shortstack{\old{7.2}\\\revised{0.9}} & \shortstack{\old{2859.5}\\\corr{5141.2}\\\revised{758.8}} \\ \hline
\end{tabular}
\vspace{-0.3cm}
\end{table}
...
\revised{In Figure\ \ref{fig:highGenus} we additionally run OptCuts to convergence on two higher-genus surfaces (buddha with genus 6 and feline with genus 2), enforcing global bijectivity constraints and a distortion bound constraint of $b_d = 4.1$.}
%\revised{Since our benchmark lacks high-genus surfaces, we test OptCuts on two extra high-genus surfaces (Figure\ \ref{fig:highGenus}). We enforce global bijectivity constraints and use $b_d = 4.1$. OptCuts robustly converges to high-quality results satisfying the distortion bounds.}

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{fig/highGenus.pdf}
\vspace{-0.6cm}
\caption{\revised{UV maps generated by OptCuts on high-genus input surfaces. Here bijectivity enforcement is enabled and the distortion bound is set to $b_d$ = $4.1$.}}
\vspace{-0.1cm}
\label{fig:highGenus}
\end{figure}
...
AutoCuts                 & N/A         & \old{4.074}\cor{4.075}        & \old{7.5789}\cor{7.704}       & \old{499.9}\cor{498.7}         \\ \hline
\multirow{2}{*}{OptCuts} & ON          & 4.073       & \old{5.8114}\corr{5.9182}\revised{5.743}       & \old{304.0}\corr{344.4}\revised{105.7}         \\
                         & OFF         & \old{4.073}\cor{4.074}        & \old{5.3065}\corr{5.4450}\revised{5.265}       & \old{148.7}\corr{156.5}\revised{50.2}         \\ \hline
\end{tabular}
\vspace{-0.3cm}
\end{table}
...
\vspace{-0.5cm}
\caption{
\old{[plots updated]}A per-example comparison of seam lengths achieved by 
...
\multirow{2}{*}{$b_d$} & \multirow{2}{*}{OptCuts} & \multicolumn{3}{c|}{$E_{s}$} & \multicolumn{3}{c|}{time (s)} \\ \cline{3-8} 
                       &                         & avg      & min     & max      & avg       & min    & max      \\ \hline
\multirow{2}{*}{4.2}   & Standard                    & \shortstack{\old{3.819}\\\corr{3.936}\\\revised{3.816}}   & \shortstack{\old{0.080}\\\cor{0.289}}  & \shortstack{\old{14.545}\\\revised{14.579}}  & \shortstack{\old{87.0}\\\corr{92.5}\\\revised{38.4}}   & \shortstack{\old{0.3}\\\revised{0.2}} & \shortstack{\old{417.8}\\\revised{235.6}} \\
                       & EB                & \shortstack{\old{3.868}\\\cor{3.837}}   & \shortstack{\old{0.159}\\\cor{0.754}}  & \shortstack{\old{14.929}\\\revised{15.035}}  & \shortstack{\old{13.0}\\\cor{15.2}}   & 0.1 & \shortstack{\old{72.1}\\\revised{98.3}}  \\ \hline
\multirow{2}{*}{4.1}   & Standard                    & \shortstack{\old{4.709}\\\corr{4.919}\\\revised{4.783}}   & 0.752  & \shortstack{\old{17.980}\\\revised{17.786}}  & \shortstack{\old{137.5}\\\corr{144.6}\\\revised{42.5}}  & \shortstack{\old{0.9}\\\corr{4.0}\\\revised{0.4}} & \shortstack{\old{886.9}\\\revised{206.2}} \\
                       & EB               & \shortstack{\old{4.795}\\\cor{4.845}}   & 1.207  & 16.895  & \shortstack{\old{17.0}\\\cor{20.0}}   & 0.1 & \shortstack{\old{87.2}\\\revised{108.2}}  \\ \hline
\multirow{2}{*}{4.05}  & Standard                    & \shortstack{\old{6.142}\\\corr{6.416}\\\revised{6.190}}   & 0.277  & 21.566  & \shortstack{\old{213.2}\\\corr{223.3}\\\revised{57.0}}  & \shortstack{\old{3.9}\\\revised{0.4}} & \shortstack{\old{1398.1}\\\revised{245.9}}   \\
                       & EB                & \shortstack{\old{6.335}\\\cor{6.383}}   & 0.328  & 23.051  & \shortstack{\old{24.1}\\\cor{28.1}}   & 0.1 & \shortstack{\old{115.4}\\\revised{131.9}} \\ \hline
\end{tabular}
\vspace{-0.3cm}
\end{table}
