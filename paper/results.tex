...
In the following sections we evaluate OptCuts and a range of other UV parameterization methods and commercial tools on meshes from a benchmark mesh dataset containing a diverse range of \old{71} \revised{\todo{Minchen add correct number:}???} surfaces commonly employed in geometry processing research. Meshes in this benchmark are organic shapes with intricate geometric details and varying resolutions ranging from 80 to 10K vertices (with 3.7K average vertex count); we also employ a set of refined Lucy models with resolutions increasing from 2.5K to 48K vertices.
...
\revised{In Figure\ \ref{fig:highGenus} we additionally run OptCuts to convergence on two higher-genus surfaces (\todo{Minchen add genus here.}), enforcing global bijectivity constraints and a distortion bound constraint of $b_d = 4.1$.}
