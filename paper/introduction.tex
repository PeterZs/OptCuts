...
%
The usability of these embeddings is highly dependent on two interconnected factors: the surface distortion introduced by the mapping and the \old{length of the }surface cuts, forming seams across which the mapping is discontinuous~\cite{Hormann2008}. \old{Both high distortion and longer seams are detrimental to downstream applications.}\revised{Downstream applications generally require low-distortion map with short seams, some even prefer invisible seams and that reflect symmetry of the shape.} Yet, reducing distortion below a desired 
